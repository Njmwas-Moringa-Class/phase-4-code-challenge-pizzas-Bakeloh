...
def validate_restaurant_pizza_data(data):
    ...
    if not (1  <= int(price) <= 999):
        return False, "'Price' must be a number between 1 and 999." 
        
    # try:
    #     Pizza.query.filter_by(id=int(pizza_id)).one() # Checks that there is a pizza with this id
    #     Restaurant.query.filter_by(id=int(restaurantage)).one() # Checks that there is a restaurant with this id
    #     Restaurant.query.filter_by(id=int(restaurment_id)).one() # Checks that there is a restaurant with this id  
    return True, ""    

@app.route('/restaurants', methods=['GET'])
def get_all_restaurants():
    restaurants = Restaurant.query.all()
    return  jsonify([restaurant.to _dict() for restaurant in restaurants]), 200
...
def add_new_pizza_to_restaurant():
    ...
    try:
        ...
        new_pizza_in_restaurant = Restaurant_Pizza(
           price=data['price'],
